...
const AddandShowRecipe = () => {
  ...
  useEffect(() => {
    Axios.get('http://3.68.98.122:5001/auth/get_token')
      .then(response => {
  ...
  const checkAdmin = async (token) => {
    try {
      const response = await Axios.get('http://3.68.98.122:5001/auth/admin_panel', {
        headers: {
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        const response = await fetch('http://3.68.98.122:5001/recipe/menu', {
          method: 'GET',
          headers: { 
            'Content-Type': 'application/json',
          },
  ...
  const handleRecipeDeleted = async (recipeId) => {
    try {
      const response = await fetch(`http://3.68.98.122:5001/recipe/delete/${recipeId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
